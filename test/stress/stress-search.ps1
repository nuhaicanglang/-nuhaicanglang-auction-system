# Day 28 - ES search API stress test (PS 5.1 compatible)
param(
    [string]$BaseUrl = "http://localhost:8080",
    [int]$Concurrent = 100,
    [int]$TotalRequests = 1000,
    [int]$PageSize = 10,
    [string[]]$Keywords = @("国画", "瓷器", "书法", "玉器", "邮票")
)
...
Write-Host "=== Day 28 Search Stress Test ===" -ForegroundColor Cyan
Write-Host "BaseUrl=$BaseUrl Concurrent=$Concurrent TotalRequests=$TotalRequests PageSize=$PageSize Keywords=$($Keywords -join ',')"
...
while ($sent -lt $TotalRequests) {
    ...
    for ($i = 0; $i -lt $batch; $i++) {
        $seq = $sent + $i + 1
        $kw = $Keywords[($seq - 1) % $Keywords.Count]
        $page = (($seq - 1) % 5) + 1
        $sort = "relevance"
        if ($seq % 4 -eq 0) { $sort = "currentPrice" }
        $encodedKw = Encode $kw
        $url = "$BaseUrl/api/search/items?keyword=$encodedKw&page=$page&size=$PageSize&status=3&sort=$sort"
        if ($seq % 7 -eq 0) {
            $url = "$BaseUrl/api/search/suggest?prefix=$encodedKw&size=10"
        }
